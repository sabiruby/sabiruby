...
作成 2026-09-26。**済み（2026-09-26）**: 段階 1・2 を main に取り込んだ。`sort { }` は著者の判断で名指しの境界のまま（+8.1% を払わない）、残った +5.4% は命令数が同じでコードの配置の揺れとして受け入れた（`../verification/bench.md`）。段階 3 は未定。著者の判断（2026-09-26）: 「いつでも sleep できるように VM を直す方向でいきたい」。段階は 1 と 2 をやる（段階 3 は後で決める）。「通常時の性能の劣化がないようにしたい」。
