...
## 8. 追記（同日）— 本家との差を「意図した逸脱」として記録する

マージ後に著者の判断が届いた。**本家の mruby-task 自体が間違っていることはありうるので、
SabiRuby 側が明らかに正しいなら、本家に合わせるために漏れを温存しなくてよい。
ただし意図した逸脱として理由つきで `docs/design/gems.md` に記録し、mrbtest の 3 基準は守ること。
本家のテストが漏れる振る舞いを assert しているなら、直し方を曲げずに、どのテストかを報告すること。**

§4b でやったことはこの判断のとおりだったので、修正そのものは変えていない。足したのは 2 つ。

**本家のテストで dormant を assert しているものはあるか — 無い。** 3 ファイルを当たった。
`gem_task.rb` で DORMANT が出るのは 2 か所だけで、どちらも「`Task#status` が返す記号の集合に
含まれるか」「`inspect` の文字列に READY か DORMANT が入るか」であって、
**dormant キューの中身も、終わったタスクが生き残ることも読んでいない**
（`gem_task.rb:57`、`:76`）。`gem_gc_task.rb` の 100 行目の DORMANT はコメントである。
`gem_queue.rb` には 1 件も無い。`Task.stat` を読むテスト（`gem_task.rb:147`）は
4 つのキーが存在して `count` が整数・`tasks` が Array であることしか見ない。
実測でも mrbtest の 3 基準（既定・`--bytes`・`--no-regexp`）は 1 行も変わっていない。
**本家のテストを曲げる必要はどこにも無かった。**

**本家側の欠陥として queue に積んだ。** `docs/verification/upstream-pr-candidates.md` の項目 4。
「How far mruby-task may drift」の規則 2 が「本家が間違って見えるなら、黙って分岐するのではなく
issue か patch にする」と言っている以上、逸脱だけ書いて本家側を放置するのは筋が通らない。
`mrb_task_mark_all`(94) が 4 本全部を mark すること、`task_create_common`(938) の
`mrb_gc_register` が二重のピン留めになっていること、`mrb_close_task`(1692) しか外す道が無いこと、
Ruby で見える再現（`Task.stat[:dormant][:count]` が減らない）、直し方 2 案、
そして「どのテストも assert していない」ことを書いた。

`docs/design/gems.md` の該当箇所にも「**本家が間違っているほうだと見なす**（著者判断）」
「本家のテストのどれも区別できない」「項目 4 に出した」の 3 つを足した。
逸脱の理由が「こちらのほうが便利だから」ではなく
「本家の振る舞いが長命なホストでは壊れているから」であることを、後から読む人が取り違えないように。
