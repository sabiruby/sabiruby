...
# 追記: 第 2 段（`pm_visit_node`）を足す

最初の版を報告したところ、著者の判断は「family-mruby と同じ第 2 段を足す」だった。理由が
具体的で、こちらの見落としを突いていた。**箱庭の脚本は `.` も `def` も付かない呼び出しが
主役**なのだ。
...
字句だけの版でこの 4 行に付く色は、`:all` の `:` 1 文字、`"season"`、`0.5`、`60`、`60.0`、
`do` だけ（実測 `0000050000022222222000` ほか、下の表）。**行頭のいちばん目立つ語が
全部 既定色**になる。エディタとして見たとき、それは色付けを
入れた意味がほとんど無い。「塗り間違えないこと」を優先して「いちばん塗りたいものを
塗らない」を選んでしまっていた。

## 移したもの

`syntax_highlight.c` の第 2 段をそのまま移した。`highlight_region()` と
`highlight_visit_node()`、それに `pm_parse` の直後の `pm_visit_node(root, ..., &hd)` の 1 行。
`highlight_data_t` に `source`（ポインタ演算の基準）を戻した。

* `PM_CALL_NODE` → `message_loc` を 8。ただし `PM_CALL_NODE_FLAGS_VARIABLE_CALL` が立って
  いるものは飛ばす。これは「レシーバも引数も括弧も無い裸の識別子」で、`x = 1` した後の
  `p x` の `x` がそれ。パーサから見れば呼び出しだが、読む人には変数に見える。
* `PM_DEF_NODE` → `name_loc` を 8。演算子でも `[]` でも名前は名前。
* `PM_SYMBOL_NODE` → `opening_loc.start` から `value_loc.end`（`closing_loc` があればそちら）
  までを 5。

順序は family-mruby と同じで、**第 2 段が第 1 段を上書きする**。`pm_visit_node` は
「親を見てから子へ降りる」ので、`me[:Hunger]` では先に `[]` 呼び出しが `[:Hunger]` を 8 に
塗り、あとから子の `PM_SYMBOL_NODE` が `:Hunger` を 5 に塗り直す（`00855555558`）。
この順序に依存しているのは面白いところで、逆順だと `:Hunger` が 8 になる。

## 「直前トークン」の判定は消した — 測ってから

著者の指示は「残してよいが、第 2 段があれば要らないなら消して単純にする（理由を worklog に）」
だった。要るかどうかは**測って決めた**。同じ shim を 2 つ作り（`prev` の判定あり／なし）、
同じ入力集合を両方に食わせて `diff` を取った。

1 回目、普通の入力と打鍵途中の入力 **48 件**: `diff` **差分なし**。`me.ea`、`def fo`、
`if x\n  me.eat(\n`、`class Foo\n  def bar\n    baz.qux\n` のような書きかけでも、
`PM_CALL_NODE` / `PM_DEF_NODE` は建つので第 2 段が塗る。

2 回目、わざと壊した入力 **14 件**（閉じない文字列の中、`%w[` の中、`=begin` の中、
引数リストが閉じない、など）: 差が出たのは **2 件だけ**。

| 入力 | prev あり | prev なし |
|---|---|---|
| `def .foo\nend` | `111088880111` | `111080000111` |
| `def foo(a,\n  b.c` | `1110888000000008` | `1110888000000000` |

1 つ目は `def .foo` という**文法的に意味の無い**入力で、prev ありは `.foo` を
メソッド名として塗る。これは「塗り足りない」ではなく**塗り間違い**で、prev の判定を残す
理由にはならない。2 つ目は「引数リストが閉じていないので本体の木が建たず、最後の `c` が
1 文字だけ無色になる」。打鍵の途中で 1 識別子が一瞬色を失うだけ。

**消した。** 48 件で差が無く、14 件の悪意ある入力でも差は 2 件、うち 1 件は prev 側が
間違えている。対して消すと `highlight_data_t` から 1 フィールド、コールバックから
改行・コメントを飛ばす `switch` ごと 15 行が消えて、コールバックは
「型 → 分類、0 でなければ塗る」だけになる。戻したくなったらこの節の 15 行を書き直せばよい。

## 何が変わったか（実測）

| 入力 | 字句だけ | 第 2 段あり |
|---|---|---|
| `tell :all, "season", s` | `0000050000022222222000` | `8888055550022222222000` |
| `sleep 0.5` | `000000444` | `888880444` |
| `every 60 do\nend` | `000000440110111` | `888880440110111` |
| `day_length 60.0` | `000000000004444` | `888888888804444` |
| `world do\nend` | `000000110111` | `888880110111` |
| `plants.each do \|p\|\nend` | `0000000888801100000111` | 同じ（`each` は `.` の後なので字句でも出ていた） |
| `me[:Hunger]` | `00056666660` | `00855555558` |
| `p 1` | `004` | `804` |
| `x = 1\np x` | `000040000` | `000040800`（`p` だけ 8、`x` は変数のまま） |
| `def ==(o)\nend` | `1110000000111` | `1110880000111` |
| `def [](i)\nend` | `1110000000111` | `1110880000111` |
| `:Plant` | `566666` | `555555` |
| `:"quoted sym"` | `5522222222222` | `5555555555555` |
| `f(key: 1, :sym => 2)` | `00555504005000000040` | `80555504005555000040` |
| `x =~ /re/` | `000002222` | `008802222` |
| `%w[a b] + %i[c]` | `222222200022222` | `222222208022252` |
| `def foo(\n  @a = :x\n` | `1110888000077000500` | `1110888000077000550` |
| `# 甲虫は歩く\np 1\n` | `3333333333333333330040` | `3333333333333333338040` |
| `def\n` | `1110` | `1118` |
| `あ.い` | `0000888` | 同じ |

（左の列も推測ではない。コミット済みの字句だけの版を `git show HEAD:compiler/csrc/shim.c`
で一時的に戻し、同じ入力を食わせて取った。）

測って初めて気づいた 3 つ。

* **演算子は 8 になる。** `x =~ /re/` の `=~`、`%w[] + %i[]` の `+`。Ruby では演算子も
  メソッド呼び出しで、`PM_CALL_NODE.message_loc` がそこを指す。family-mruby でも同じ。
  エディタの色としては「演算子が名前と同じ色」になるが、これは移した版の素の挙動なので
  そのままにした。H2 で分類 8 の色を選ぶとき、演算子がその色で出ることを見込んでおくこと。
* **`%i[c]` の要素は 5 になる**（`222222208022252` の `c` が 5）。`%i` はシンボルの配列
  なので、要素が `PM_SYMBOL_NODE` として木に建つ。文字列リテラル風の外枠が 2 で中身が 5
  という、字句だけでは出ない塗り分け。
* **`def\n` だけを書くと改行が 8 になる**（`1118`）。名前が欠けた `PM_DEF_NODE` の
  `name_loc` が改行を指すため。改行に色が付いても描画には出ないので放置した。

構文エラーの入力は第 2 段を足しても壊れない。`def foo(\n  @a = :x\n` は
`1110888000077000550` で、`foo` は 888 のまま、`:x` は第 2 段が 55 にした。

## 速さ（第 2 段の代金）

同じ機械、release、best of 20（中央値も併記）。第 2 段は木を 1 回歩く分だけ増える。

| 入力 | バイト | 字句だけ best | 第 2 段あり best | median |
|---|---:|---:|---:|---:|
| `prelude.rb` | 21433 | 198.8 µs | 221.4 µs | 242.5 µs |
| `world_prelude.rb` | 17161 | 91.8 µs | 102.5 µs | 110.8 µs |
| `world.rb` | 16249 | 136.3 µs | 147.4 µs | 159.5 µs |
| `creatures/beetle.rb` | 5870 | 41.5 µs | 47.4 µs | 99.3 µs |
| `creatures/rabbit.rb` | 3632 | 59.3 µs | 76.2 µs | 83.5 µs |
| 5 つ連結 | 64345 | 580.3 µs | 678.6 µs | 981.1 µs |

**+8〜17%**。エディタが持つ最大の `world.rb` で 147 µs、60 fps の 1 フレーム（16.7 ms）の
0.9%。既定 5（本文が変わったときだけ）を守る限り、上限を置かない判断は変わらない。
（beetle.rb の best と median の開きが大きいのは、この機械で別の担当が動いている可能性が
あるため。判断に効く桁ではないので取り直していない。）

## テスト

`compiler/tests/highlight.rs` を 11 本 → **12 本**にした。期待値はすべて取り直して実測で
差し替え、箱庭の形の節（`the_garden_scripts_bare_calls`）を新設した。著者が名指しした
`p 1` の `p`、`def ==` の `==`、`tell :all, "x"` の `tell` と `:all`、`me[:Hunger]` の `[]`、
`plants.each` の `each`、`world do` の `world` は全部テストに入っている。
