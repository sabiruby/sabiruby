...
takes a block reads `direct_send` once more; `OP_GETIDX` on a Hash looks the key up itself and
leaves only a miss with a default proc to the send. The measurements are in the worklog
(**pending**: the machine was busy when this was written).
